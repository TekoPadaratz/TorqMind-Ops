...
`companies`, `branches`, `sectors`, `auth_users`, `password_change_events` (V15), `routine_templates`, `routine_runs`, `occurrences`, `fuel_quality_analyses` (V14), `notifications`, `task_comments`, `task_attachments`, `task_activities`, `voice_drafts` (V12), `company_settings` (V16), `email_settings` (V22), `push_vapid` + `push_subscriptions` (V23)
