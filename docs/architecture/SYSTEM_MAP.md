...
| API pública v1 | `interfaces/rest/publicapi/PublicApiController` |
...
`companies`, `branches`, `sectors`, `auth_users`, `password_change_events` (V15), `routine_templates`, `routine_runs`, `occurrences`, `fuel_quality_analyses` (V14), `notifications`, `task_comments`, `task_attachments`, `task_activities`, `voice_drafts` (V12), `company_settings` (V16), `email_settings` (V22), `push_vapid` + `push_subscriptions` (V23), `api_keys` (V24), `webhooks` (V25)
...
Troca de senha: `POST /api/auth/password` (própria senha; senha atual errada = 422). MASTER redefine via `PUT /api/admin/users/{id}/password`. Ambas sobem `password_epoch` e gravam `password_change_events` (`CREATED|SELF_CHANGE|ADMIN_RESET`) sem guardar a senha. **Recuperação por e-mail**: `POST /api/auth/password/forgot` (envia link com token de uso único, 1h) + `POST /api/auth/password/reset`; usuário tem campo `email`; envio best-effort (`EmailService`/SMTP configurável; sem SMTP apenas registra). E-mail também em atraso/escalonamento. **Config SMTP/remetente**: perfil singleton `email_settings` (senha cifrada) via `GET/PUT /api/admin/email-settings` + teste `POST /api/admin/email-settings/test` (MASTER); `EmailSettingsService.resolveRuntime` usa o banco, senão o ambiente.
...
Headers de segurança (SecurityConfig): `frameOptions sameOrigin`, HSTS, Referrer-Policy. Dashboard gerencial: `GET /api/dashboard/metrics` (conclusão no prazo %, aging de atrasos, ranking por filial). **Relatório gerencial (PDF)**: `GET /api/dashboard/report.pdf?from=&to=` (rotinas agendadas + ocorrências abertas no período, KPIs, envelhecimento, ranking por filial e lista de atrasos; `ManagementReportPdfRenderer`). Export: `GET /api/routines/runs/export.csv` e `GET /api/occurrences/export.csv` (BOM UTF-8, ';'). Calendário: `GET /api/routines/runs/calendar?from=&to=` (agenda mensal agrupada por dia). Lote: `POST /api/routines/templates/bulk-delete` (excluir rotinas, MASTER/OWNER — soft-delete por item, ignora sem permissão) e `POST /api/routines/runs/bulk-reassign` (reatribuir tarefas **em aberto**, MASTER/OWNER/MANAGER — valida usuário destino, pula runs de outra empresa/filial, já concluídas/rejeitadas ou já atribuídas ao destino; retorna `{reassigned, skipped}` e grava atividade `REASSIGNED` "de X para Y"). Na UI ambas exigem um **modo explícito** e a reatribuição só deixa marcar tarefas em aberto (concluídas/rejeitadas ficam esmaecidas). Frontend é **PWA instalável** (manifest + ícone; sem SW de cache). **Notificações push (Web Push/VAPID)**: `GET /api/push/public-key` + `POST /api/push/subscribe|unsubscribe` (`WebPushService` gera/persiste o par VAPID — chave privada cifrada — e envia best-effort em thread separada; `PushController`). Service Worker **só-push** `frontend/public/sw-push.js` (sem handler `fetch`, não cacheia nada; respeita a decisão de "sem SW de cache"). Opt-in por dispositivo em Conta; no iOS exige o app instalado na tela de início. **API pública v1 (somente leitura)**: `GET /api/public/v1/{routines/runs?from=&to= | occurrences?status= | dashboard/summary}` autenticada por header `X-API-Key` (`ApiKeyAuthFilter` valida o hash, escopa à empresa da chave, rate-limit 120/min); chaves geridas pelo MASTER (`AdminController` /api/admin/api-keys, `ApiKeyService` — chave mostrada 1x, guardada só como hash SHA-256). **Webhooks de saída**: MASTER cadastra em `/api/admin/webhooks`; a cada evento (rotina/ocorrência) o `WebhookService` (disparado pelo `NotificationService`) faz POST **assinado** (HMAC-SHA256 no header `X-TorqMind-Signature: sha256=...`) best-effort em thread separada, com **`SsrfGuard`** (só https, bloqueia IP interno/privado/loopback/link-local/metadados, re-checa no envio) + dedup de 10s + registro de status. **i18n (frontend)**: `i18n.tsx` (`LanguageProvider` + `useI18n().t(key, vars?)`, dicionários planos PT-BR/EN com interpolação `{var}`, idioma persistido em `localStorage` e refletido em `document.documentElement.lang`, seletor em Conta). Cobertura **completa** de todas as telas: chrome (nav/header), Login, Radar/Dashboard, Avisos, Conta (senha/2FA/push), Redefinir senha, Rotinas (form + agenda + lote/reatribuição), Calendário (meses/datas localizados por `toLocaleDateString`), Ocorrências, detalhes de Rotina e Ocorrência, Thread (evidências/histórico), e toda a Gestão (Empresa/Filial/Setor, Usuários, Configurações, E-mail, API pública, Webhooks) e a Análise de qualidade de combustível. Enums traduzidos por chave (`status.*`, `ostatus.*`, `prio.*`, `rec.*`, `target.*`, `wd.*`). **Exceção intencional**: o `VoiceSheet` (comando por voz) permanece em PT-BR porque todo o pipeline de voz é pt-BR (reconhecimento do navegador + interpretador determinístico no backend + TTS); traduzir só a UI induziria comandos que o backend não entende. i18n de voz depende de NLU multilíngue no backend (trabalho futuro).
