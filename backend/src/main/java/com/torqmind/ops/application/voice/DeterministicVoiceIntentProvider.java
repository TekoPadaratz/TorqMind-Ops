...
public class DeterministicVoiceIntentProvider implements VoiceIntentProvider {
    ...
    private static final Pattern TASK_TITLE = Pattern.compile("tarefa (?:de |d[aeo] )?(.+?)(?: para | amanh| hoje| toda| nos dias| exig| venc|$)", Pattern.CASE_INSENSITIVE);
    private static final Pattern FOR_USER = Pattern.compile("para o (?:gerente |funcion[aá]rio |dono )?([^,]+?)(?: do | da | amanh| hoje| exig|$)", Pattern.CASE_INSENSITIVE);
    ...
    public VoiceIntent interpret(String transcript, VoiceContext context) {
        ...
        } else if (containsAny(low, "mostre", "liste", "minhas tarefas")) {
    ...
    private static void fillCreateTask(VoiceIntent intent, String t, String low) {
        ...
        if (low.contains("toda segunda") || low.contains("todas as segundas") || low.contains("semanal")) {
        ...
        intent.setRequiresPhoto(containsAny(low, "foto", "fotografia", "exigindo foto"));
        intent.setRequiresComment(containsAny(low, "coment"));
