...
public class RoutineService {
    ...
    public RoutineRun transition(Long runId, RoutineStatus next, String comment, AppUserPrincipal me) {
        ...
        if (next == RoutineStatus.CONCLUIDA && template != null && template.isRequiresPhoto()) {
            long photoCount = run.getAssignedUserId() == null
                    ? attachmentRepository.countByTaskTypeAndTaskIdAndMimeTypeStartingWith(
                            TaskType.ROUTINE_RUN.name(), run.getId(), "image/")
                    : attachmentRepository.countByTaskTypeAndTaskIdAndUploadedByAndMimeTypeStartingWith(
                            TaskType.ROUTINE_RUN.name(), run.getId(), run.getAssignedUserId(), "image/");
            if (photoCount == 0) {
